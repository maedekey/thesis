...
class FrodoKEM(object):
    ...
    def __print_intermediate_value(self, name, value):
        ...
        if isinstance(value, bytes):
            ...
            print()
        elif name in ["A", "B", "B'", "B''", "B'S", "C", "C'", "E", "E'", "E''", "M", "S", "S'", "S^T", "V",
                      "mu_encoded"]:
    ...
    def kem_keygen(self):
        ...
        z = bytes(s_seedSE_z[
                  self.len_s_bytes + self.len_seedSE_bytes: self.len_s_bytes + self.len_seedSE_bytes + self.len_z_bytes])
    ...
    def kem_encaps(self, pk):
        ...
        rbytes = self.shake(bytes(b'\x96') + seedSE,
                            (2 * self.mbar * self.n + self.mbar * self.mbar) * self.len_chi_bytes)
        ...
        Eprimeprime = self.sample_matrix(r[2 * self.mbar * self.n: 2 * self.mbar * self.n + self.mbar * self.nbar],
                                         self.mbar, self.nbar)
    ...
    def kem_decaps(self, sk, ct):
        ...
        rbytes = self.shake(bytes(b'\x96') + seedSEprime,
                            (2 * self.mbar * self.n + self.mbar * self.mbar) * self.len_chi_bytes)
        r = [struct.unpack_from('<H', rbytes, 2 * i)[0] for i in range(2 * self.mbar * self.n + self.mbar * self.nbar)]
        self.__print_intermediate_value("r", r)
